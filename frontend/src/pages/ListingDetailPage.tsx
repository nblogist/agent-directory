  ...
  if (isLoading) {
    return (
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-20 py-8 animate-pulse">
        <div className="h-4 w-48 bg-dark-surface rounded mb-8" />
        <div className="flex flex-col md:flex-row gap-6 items-start mb-10">
          <div className="size-32 rounded-xl bg-dark-surface" />
          <div className="space-y-3 flex-1">
            <div className="h-8 w-64 bg-dark-surface rounded" />
            <div className="h-4 w-96 bg-dark-surface/60 rounded" />
            <div className="flex gap-2">
              <div className="h-6 w-20 bg-dark-surface rounded" />
              <div className="h-6 w-20 bg-dark-surface rounded" />
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="h-48 bg-dark-surface/40 rounded-xl" />
          </div>
        </div>
      </main>
    );
  ...
  if (error) {
    ...
    return (
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-20 py-20 text-center">
        <span className="material-symbols-outlined text-6xl text-theme-text-muted mb-4">{is404 ? 'error_outline' : 'cloud_off'}</span>
  ...
  return (
    <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-20 py-8">
      ...
      <nav className="flex items-center gap-2 mb-8 text-sm font-medium">
        ...
        <Link className="text-theme-text-muted hover:text-primary transition-colors" to="/browse">Directory</Link>
        <span className="text-theme-text-muted material-symbols-outlined text-xs">chevron_right</span>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-10">
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
          ...
          <div>
            ...
            <p className="text-theme-text-secondary text-lg mb-4">{listing.short_description}</p>
                ...
                <Link
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    chain.is_featured
                      ...
                      : 'bg-dark-surface text-theme-text-secondary'
        ...
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
          {listing.api_endpoint_url && (
            <a
              ...
              className="flex-1 lg:flex-none flex items-center justify-center gap-2 rounded-lg h-11 px-6 bg-accent text-on-accent text-sm font-bold hover:opacity-90 transition-opacity shadow-[0_0_15px_rgba(0,242,255,0.2)]"
            >
          ...
          {listing.docs_url && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 lg:flex-none flex items-center justify-center gap-2 rounded-lg h-11 px-6 bg-dark-surface2 text-theme-text text-sm font-bold hover:bg-dark-border transition-colors"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <section>
            ...
            <div className="bg-dark-surface border border-dark-border p-8 rounded-xl">
              ...
              <div className="flex flex-wrap gap-2">
                {listing.tags.map(tag => (
                  <Link
                    ...
                    className="px-3 py-1.5 rounded-lg bg-dark-surface border border-dark-border text-theme-text-secondary text-sm hover:bg-primary/10 hover:text-primary transition-colors"
                  >
                    {tag.name}
                  </Link>
                ))}
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-dark-surface border border-dark-border rounded-xl overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div className="flex items-center justify-between">
                <span className="text-sm text-theme-text-muted">Views</span>
                <span className="text-sm font-bold flex items-center gap-1">
                  <span className="material-symbols-outlined text-sm text-theme-text-secondary">visibility</span>
                  {listing.view_count.toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-theme-text-muted">Reputation</span>
                {listing.reputation_score != null ? (
                  <span className="text-sm font-bold text-primary">{listing.reputation_score}</span>
                ) : (
                  <span className="text-sm text-theme-text-muted" title="Reputation scoring coming soon.">N/A</span>
                )}
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-theme-text-muted">Submitted</span>
              ...
              {listing.approved_at && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-theme-text-muted">Approved</span>
                  <span className="text-sm text-theme-text-secondary">
              ...
              {listing.updated_at && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-theme-text-muted">Updated</span>
                  <span className="text-sm text-theme-text-secondary">
              ...
              {listing.api_endpoint_url && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-theme-text-muted">API Endpoint</span>
          ...
          <div className="bg-dark-surface border border-dark-border rounded-xl p-6 text-center">
            <span className="material-symbols-outlined text-4xl mb-3 text-primary">auto_awesome</span>
            <h5 className="text-lg font-bold mb-2 text-theme-text">Want your tool listed?</h5>
            <p className="text-theme-text-secondary text-sm mb-4">Reach developers and builders in the AI agent ecosystem.</p>
            <Link to="/submit" className="block w-full bg-primary text-white font-bold py-3 rounded-lg text-sm hover:bg-primary/90 transition-all">
              Submit a Listing
            </Link>
          </div>
        </div>
