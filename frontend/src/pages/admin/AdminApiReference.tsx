...
function Endpoint({ method, path, description }: { method: string; path: string; description: string }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2">
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shrink-0 ${colors[method] ?? 'bg-dark-surface2 text-theme-text-secondary'}`}>
        {method}
      </span>
      <code className="text-xs text-theme-text font-mono shrink-0">{path}</code>
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <section className="mb-8">
        ...
        <div className="bg-dark-surface border border-dark-border rounded-xl p-4 divide-y divide-dark-border">
